          ...
          {services.map((service, index) => (
            ...
            >
              <Link href={service.href} className="group block h-full">
                <div className="relative flex h-full flex-col overflow-hidden rounded-2xl bg-(--color-brand-surface) p-8 shadow-sm transition-all duration-300 hover:-translate-y-2 hover:shadow-md border border-transparent hover:border-(--color-brand-gold-pale)">
                  <div className="mb-6 inline-flex h-16 w-16 items-center justify-center rounded-2xl bg-(--color-brand-gold-pale) text-(--color-brand-gold) transition-colors group-hover:bg-(--color-brand-gold) group-hover:text-white">
                    <service.icon className="h-8 w-8" />
                  </div>
                  <h3 className="mb-3 text-2xl font-bold text-(--color-brand-navy) group-hover:text-(--color-brand-gold) transition-colors">
                    {service.title}
                  </h3>
